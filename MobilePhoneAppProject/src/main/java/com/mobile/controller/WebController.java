...
public class WebController {
	...
	//device 등록 
	@Transactional
	@RequestMapping("/admin/deviceUpdate")
	public String deviceUpdate(Device device, MultipartFile file , Model model) {
		...
		try {
			if(file!=null) {
				imgPath = s3Service.deviceUpdate(file, device.getDeviceId());
			}
		...
		productService.deviceUpdate(device, ext);
