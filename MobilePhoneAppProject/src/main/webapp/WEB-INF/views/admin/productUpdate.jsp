						...
						<div>
							...
							<div class="form-group col-md-2">
								<label for="inputState">통신사</label> <select id="carrierId"
									...
									<option value='1'
										<c:if test="${products.carrier.carrierId == '1'}"> selected="selected" </c:if>>SKT
									</option>
									<option value='2'
										<c:if test="${products.carrier.carrierId == '2'}"> selected="selected" </c:if>>KT
									</option>
									<option value='3'
										<c:if test="${products.carrier.carrierId == '3'}"> selected </c:if>>LGU+
									</option>
								</select>
							</div>
							...
							<div class="form-group col-md-2">
								<label for="inputState">디바이스 이름</label> <select id="deviceId"
									...
									<c:forEach items="${devices}" var="devices">
										<c:choose>
											<c:when
												test="${devices.deviceId == products.device.deviceId}">
							...
							<div class="form-group col-md-2">
								<label for="inputState">요금제 이름</label> <select
									...
									<c:forEach items="${callingPlans}" var="callingPlans">
										<c:choose>
											<c:when
												test="${callingPlans.callingPlanId == products.callingPlan.callingPlanId}">
												<option value="${callingPlans.callingPlanId}"
													selected="selected">${callingPlans.planName}</option>
											</c:when>
											<c:otherwise>
							...
							<div class="form-group col-md-2">
								<label for="inputEmail4">개통유형</label> <select
									...
									<option value='1'
										<c:if test="${products.activationType == '1'}"> selected="selected" </c:if>>기기변경
									</option>
									<option value='0'
										<c:if test="${products.activationType == '0'}"> selected="selected" </c:if>>번호이동</option>
								</select>
							</div>
							...
							<div class="form-group col-md-2">
								<label for="inputEmail4">공시지원금</label> <input type="text"
									class="form-control" id="mainSupportFund"
									value="${products.mainSupportFund}" name="mainSupportFund">
							</div>

							<div class="form-group col-md-2">
								<label for="inputEmail4">마켓지원금</label> <input type="text"
									class="form-control" id="marketSupportFund"
									value="${products.marketSupportFund}" name="marketSupportFund">
							</div>
